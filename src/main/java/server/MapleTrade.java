...
public class MapleTrade{
	...
	private void lock(){
		locked = true;
		partner.getChr().getClient().announce(MiniRoomBase.Trade.getTradeConfirmation());
	}
	...
	private void complete2(){
		...
		for(Item item : exchangeItems){
			if((item.getFlag() & ItemConstants.KARMA) == ItemConstants.KARMA) item.setFlag((byte) (item.getFlag() ^ ItemConstants.KARMA)); // items with scissors of karma used on them are reset once traded
			else if(item.getType() == 2 && (item.getFlag() & ItemConstants.SPIKES) == ItemConstants.SPIKES) item.setFlag((byte) (item.getFlag() ^ ItemConstants.SPIKES));
		...
		chr.getClient().announce(MiniRoomBase.Trade.getTradeCompletion(number));
	}
	...
	private void cancel(){
		...
		chr.getClient().announce(MiniRoomBase.Trade.getTradeCancel(number));
	}
	...
	public void setMeso(int meso){
		...
		if(chr.getMeso() >= meso){
			...
			this.meso += meso;
			chr.getClient().announce(MiniRoomBase.Trade.getTradeMesoSet((byte) 0, this.meso));
			if(partner != null){
				partner.getChr().getClient().announce(MiniRoomBase.Trade.getTradeMesoSet((byte) 1, this.meso));
			}
		}else{}
	}
	...
	public void addItem(Item item){
		items.add(item);
		chr.getClient().announce(MiniRoomBase.Trade.getTradeItemAdd((byte) 0, item));
		if(partner != null){
			partner.getChr().getClient().announce(MiniRoomBase.Trade.getTradeItemAdd((byte) 1, item));
		}
	}

	public void chat(String message){
		BigBrother.trade(chr, partner.getChr(), message);
		chr.getClient().announce(MiniRoomBase.Trade.getTradeChat(chr, message, true));
		if(partner != null){
			partner.getChr().getClient().announce(MiniRoomBase.Trade.getTradeChat(chr, message, false));
		}
	}
	...
	private boolean fitsInInventory(){
		...
		for(Map.Entry<MapleInventoryType, Integer> entry : neededSlots.entrySet()){
			if(chr.getInventory(entry.getKey()).isFull(entry.getValue() - 1)) return false;
		}
		return true;
	}
	...
	public static void startTrade(MapleCharacter c){
		if(c.getTrade() == null){
			...
			c.getClient().announce(MiniRoomBase.Trade.getTradeStart(c.getClient(), c.getTrade(), (byte) 0));
		}else{
	...
	public static void inviteTrade(MapleCharacter c1, MapleCharacter c2){
		if(c2.getTrade() == null){
			...
			c2.getClient().announce(MiniRoomBase.Trade.getTradeInvite(c1));
		}else{
	...
	public static void visitTrade(MapleCharacter c1, MapleCharacter c2){
		if(c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null && c2.getTrade().getPartner() == c1.getTrade()){
			c2.getClient().announce(MiniRoomBase.Trade.getTradePartnerAdd(c1));
			c1.getClient().announce(MiniRoomBase.Trade.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1));
