...
public enum MapleMiniDungeon{
	...
	public static boolean isDungeonMap(int map){
		for(MapleMiniDungeon dungeon : MapleMiniDungeon.values()){
			if(map >= dungeon.getDungeonId() && map <= dungeon.getDungeonId() + dungeon.getDungeons()) return true;
		}
		return false;
	}
	...
	public static MapleMiniDungeon getDungeon(int map){
		for(MapleMiniDungeon dungeon : MapleMiniDungeon.values()){
			if(map >= dungeon.getDungeonId() && map <= dungeon.getDungeonId() + dungeon.getDungeons()) return dungeon;
		}
		return null;
	}
}
