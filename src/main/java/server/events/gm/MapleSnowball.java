...
public class MapleSnowball{
	...
	public MapleSnowball(int team, MapleMap map){
		...
		for(MapleCharacter chr : map.getCharacters()){
			if(chr.getTeam() == team) characters.add(chr);
		}
	}
		...
		TimerManager.getInstance().schedule("startEvent", new Runnable(){
			...
			public void run(){
				if(map.getSnowball(team).getPosition() > map.getSnowball(team == 0 ? 1 : 0).getPosition()){
					for(MapleCharacter chr : characters){
						if(chr != null) chr.announce(MaplePacketCreator.rollSnowBall(false, 3, map.getSnowball(0), map.getSnowball(0)));
					}
					winner = true;
				}else if(map.getSnowball(team == 0 ? 1 : 0).getPosition() > map.getSnowball(team).getPosition()){
					for(MapleCharacter chr : characters){
						if(chr != null) chr.announce(MaplePacketCreator.rollSnowBall(false, 4, map.getSnowball(0), map.getSnowball(0)));
					}
					winner = true;
				} // Else
				warpOut();
	...
	public void hit(int what, int damage){
		if(what < 2) if(damage > 0) this.hits--;
		else{
			...
			}else this.snowmanhp -= damage;
			map.broadcastMessage(MaplePacketCreator.rollSnowBall(false, 1, map.getSnowball(0), map.getSnowball(1)));
		}
		if(this.hits == 0){
			this.position += 1;
			if(this.position == 45) map.getSnowball(team == 0 ? 1 : 0).message(1);
			else if(this.position == 290) map.getSnowball(team == 0 ? 1 : 0).message(2);
			else if(this.position == 560) map.getSnowball(team == 0 ? 1 : 0).message(3);
			this.hits = 3;
	...
	public void message(int message){
		for(MapleCharacter chr : characters){
			if(chr != null) chr.announce(MaplePacketCreator.snowballMessage(team, message));
		}
	}
	...
	public void warpOut(){
		TimerManager.getInstance().schedule("snowball-warpout", new Runnable(){
			...
			@Override
			public void run(){
				if(winner == true) map.warpOutByTeam(team, 109050000);
				else map.warpOutByTeam(team, 109050001);
