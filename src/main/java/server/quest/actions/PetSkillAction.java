...
public class PetSkillAction extends MapleQuestAction{
	...
	public PetSkillAction(MapleQuest quest, LittleEndianAccessor lea){
		super(MapleQuestActionType.PETSKILL, quest);
		questID = quest.getId();
		processData(lea);
	}
	...
	public boolean check(MapleCharacter chr, Integer extSelection){
		...
		if(!(status.getStatus() == MapleQuestStatus.Status.NOT_STARTED && status.getForfeited() > 0)) return false;
