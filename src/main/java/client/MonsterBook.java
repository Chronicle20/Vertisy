...
public final class MonsterBook{
	...
	public void addCard(final MapleClient c, final int cardid){
		c.getPlayer().getMap().broadcastMessage(c.getPlayer(), UserRemote.UserEffect.showForeginCardEffect(c.getPlayer().getId()), false);
		for(Entry<Integer, Integer> all : cards.entrySet()){
			if(all.getKey() == cardid){
				if(all.getValue() > 4){
					...
					c.announce(UserLocal.UserEffect.showGainCard());
					calculateLevel();
				}
				return;
			}
