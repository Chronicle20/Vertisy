...
public final class DueyHandler extends AbstractMaplePacketHandler{
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c){
		if(!ServerConstants.USE_DUEY) return;
		...
		if(operation == Actions.TOSERVER_SEND_ITEM.getCode()){
			...
			if(mesos < 0 || (long) mesos > Integer.MAX_VALUE || ((long) mesos + fee + getFee(mesos)) > Integer.MAX_VALUE || amount < 1){
				...
				FilePrinter.print(FilePrinter.EXPLOITS + c.getPlayer().getName() + ".txt", c.getPlayer().getName() + " tried to use duey with mesos " + mesos + " and amount " + amount + "\r\n");
				...
				if(inventId > 0){
					...
					if(item != null && c.getPlayer().getItemQuantity(item.getItemId(), false) > amount){
						if(ItemConstants.isRechargable(item.getItemId())){
							MapleInventoryManipulator.removeItem(c, inv, itemPos, item.getQuantity(), true);
						}else{
							MapleInventoryManipulator.removeItem(c, inv, itemPos, amount, true);
						}
			...
			if(dp.getItem() != null){
				if(!c.getPlayer().canHoldItem(dp.getItem())){
