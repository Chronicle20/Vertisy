...
public final class RemoteGachaponHandler extends AbstractMaplePacketHandler{
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c){
		...
		if(ticket != 5451000){
			...
			NPCScriptManager.getInstance().start(c, gacha, null);
			return;
		}else if(c.getPlayer().getInventory(ItemInformationProvider.getInstance().getInventoryType(ticket)).countById(ticket) < 1){
