...
public final class NoteActionHandler extends AbstractMaplePacketHandler{
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c){
		...
		if(action == 0 && c.getPlayer().getCashShop().getAvailableNotes() > 0){
			...
			try{
				if(c.getPlayer().getCashShop().isOpened()) c.announce(CCashShop.CashItemResult.showCashInventory(c));
			...
			for(int i = 0; i < num; i++){
				...
				try{
					...
					if(rs.next()) fame += rs.getInt("fame");
					rs.close();
			...
			if(fame > 0){
				...
				c.announce(CWvsContext.OnMessage.getShowFameGain(fame));
			}
		}
	}
}
