...
public final class ExpTable{
	...
	private static final Integer[] skillExp = new Integer[121];// 121 since 0 is null.
	static{
		double percent = 1.10405;
		int exp = 83;
		int expRequired = 0;
		for(int level = 0; level < 120; level++){
			if(level > 1) exp *= percent;
			if(level > 0) expRequired += exp;
			skillExp[level + 1] = expRequired;
		}
	}
	...
	public static int getRSSkillExpNeededForLevel(int level){
		return skillExp[level];
	}
}
