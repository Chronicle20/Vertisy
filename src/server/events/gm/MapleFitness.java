...
public class MapleFitness{
	...
	public MapleFitness(final MapleCharacter chr){
		this.chr = chr;
		this.schedule = TimerManager.getInstance().schedule("maplefitness", new Runnable(){

			@Override
			public void run(){
				if(chr.getMapId() >= 109040000 && chr.getMapId() <= 109040004) chr.changeMap(chr.getMap().getReturnMap());
			}
		}, 900000);
	}
	...
	public void checkAndMessage(){
		this.schedulemsg = TimerManager.getInstance().register("maplefitness", new Runnable(){
