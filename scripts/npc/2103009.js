/**
 *	@Name: Ariant Private House Cupboard (#1)
 *	@Quest: Sejan's Test
 */
    ...
    if (mode < 0)
        ...
        if (status == 0 && mode == 1) {
			if (cm.isQuestStarted(3929) && cm.haveItem(4031580, 1)) {
				if (cm.getPlayer().getQuestNAdd(3929).getStorage() == null || !cm.getPlayer().getQuestNAdd(3929).getStorage().contains(2103009)) {
					cm.sendOk("#b(This house looks okay. You leave the wrapped food here.)", 2);
					cm.gainItem(4031580, -1);
					cm.getPlayer().getQuestNAdd(3929).getStorage().add(2103009);
