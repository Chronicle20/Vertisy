 ...
 */
var status = 0;
var chosen = -1; //cuz we need something to determine which selection it originated from the 1st npc selection list.

function start() {
    if (cm.isLeader()) {
        var eim = cm.getEventManager("HenesysPQ").getInstance("HenesysPQ");
...
#L2# I would like to save some for Tory and/or leave this place.#k#l");
        }
    } else {
        cm.sendSimple("Growl! I am Growlie, always ready to protect this place. What brought you here?\r\n\
#b#L0# Please tell me what this place is all about.#l\r\n\
...
function action(mode, type, selection) {
    var riceCakes = 10; //number of rice cakes needed.

    if (mode < 0 || (mode === 0 && status === 0)) {
        cm.dispose();
    } else {
...
//        }
        ...
        if (status === 51) {
            ...
            if (chosen === -1) { //store selection in chosen.
                chosen = selection;
            }
            if (chosen === 0) {
                ...
                if (cm.haveItem(4001101, riceCakes) || cm.getPlayer().isGM()) {
                    cm.sendNext("Oh... isn't this rice cake made by Moon Bunny? Please hand me the rice cake.");
                } else {
                    cm.sendOk("I advise you to check and make sure that you have indeed gathered up #b"
                            + riceCakes + " #t4001101#s#k.");
                    cm.dispose();
                }
            } else if (chosen === 2) {
...
//                var eim = cm.getPlayer().getEventInstance();
            ...
            if (chosen === 0) {
                cm.sendNextPrev("I would like for you and your party members to cooperate and get me "
                        + riceCakes + "rice cakes. I strongly advise you to get me the rice cakes within the allotted time.");
            }
        } else {
            cm.dispose();
        }
...
function clearPQ(eim) {
    cm.givePartyQuestExp("HenesysPQ");
    eim = cm.getEventManager("HenesysPQ").getInstance("HenesysPQ");
