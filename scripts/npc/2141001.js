 ...
 *	@Map: Deep Place of Temple - Forgotten Twilight
...
function start() {
	expedition = cm.getExpedition(Packages.server.expeditions.MapleExpeditionType.PINKBEAN);	
	...
	var bossQuest = cm.getQuestRecord(30010);
	var bossQuest1 = cm.getQuestRecord(30011);
	...
	if (amtFought > 0 || time + (24 * 60 * 60 * 1000) >= cm.getCurrentTime()/* && !cm.getPlayer().isGM()*/) {
		cm.sendOk("You may only attempt Pink Bean once a day. Time left until allowed re-entry: " + cm.getReadableMillis(cm.getCurrentTime(), time + (12 * 60 * 60 * 1000)));
		cm.dispose();
		return;
	}
	...
	if (eim_status == null || eim_status.equals("0")) {
		if (expedition == null) { // Start expedition
			...
			cm.sendOk("I'm afraid another expedition is attempting to defeat Pink Bean. Please wait until they have finished trying.");
			cm.dispose();
		}
	} else if (expedition.isInProgress() || eim_status.equals("1")) {
		if (expedition.contains(cm.getPlayer())) { //If you're registered, warp you in
			status = 3;
			cm.sendNext("The expedition has begun! You will now be escorted into the #bTwilight of Gods#k. Good luck.");
		} else {
			cm.sendOk("I'm afraid another expedition is attempting to defeat Pink Bean. Please wait until they have finished trying.");
			cm.dispose();
			return;
...
function action(mode, type, selection) {
	...
	var customData = bossQuest.getCustomData();
	if (customData == null) {
		bossQuest.setCustomData("0");
        customData = "0";
	}
    ...
    switch (status) {
        case 0: // Register leader and expedition
            if (mode == 1) {
                cm.createExpedition(Packages.server.expeditions.MapleExpeditionType.PINKBEAN);
            }
            cm.dispose();
            break;
        case 1:
			if (selection == 0) { // View current members
				...
				cm.sendOk("The expedition will begin and you will now be escorted to #bTwilight of the Gods#k.");
				status = 6;
        ...
        case 3: // Player is in expedition: warp them back in
			em.getInstance("PinkBean_" + cm.getPlayer().getClient().getChannel()).registerPlayer(cm.getPlayer());
			cm.dispose();
            break;
        ...
        case 6: // Leader has started the expedition
            var min = 1; // 'cuz this is a private server
			var size = expedition.getMembers().size();
			if (size < min) {
			...
			em.startInstance(expedition, 30010, amtFought + 1);
			cm.dispose();
			break;
		default:
