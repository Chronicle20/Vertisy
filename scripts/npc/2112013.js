/**
 *	@Name: Investigation Result (Juliet)
 *	@Author: iPoopMagic
 */
...
function start() {
    ...
    if (!cm.canHold(4001131)) {
		cm.sendOk("You will need 1 ETC space.");
		cm.dispose();
		return;
    }
    if (cm.getPlayer().getMapId() == 926110000) { // 26 npcs
		...
		var npc = Packages.server.life.MapleLifeFactory.getNPC(2112010); // Yulete
		...
		if (npc != null) {
			...
			npc.setRx0(x + 50);
			npc.setRx1(x - 50);
			npc.setFh(player.getMap().getMapData().getFootholds().findBelow(new java.awt.Point(x, y)).getId());
