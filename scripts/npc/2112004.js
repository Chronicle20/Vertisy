...
function action(mode, type, selection) {
    ...
    if (status == 0) {
    	...
    	if (mapid == 926100000) {
    		cm.sendOk("You should try investigating around here. Look at the files in the Library until you can find the entrance to the Lab. #bJust because you looked in an area, doesn't mean you fully investigated it!#k");
    		cm.dispose();
    		...
    		if ((em.getProperty("stage4talk") == null || !em.getProperty("stage4talk").equals("done")) && cm.getPlayer().getMap().getCharacters().size() == cm.getPlayer().getEventInstance().getPlayerCount()) {
				cm.sendOk("Let us go and save my Juliet.");
		...
		if (selection == 1) {
			...
			if (party.size() < minPlayers || party.size() > maxPlayers) {
				...
				if (next) {
					var em = cm.getEventManager("Romeo");
					if (em == null) {
						cm.sendOk("Magatia PQ (Romeo) does not work.");
						cm.dispose();
					} else {
