...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if(!cm.isQuestCompleted(4576)) {
            cm.sendOk("Sorry, I don't think you have what it takes to fight Scarlion or Targa yet.");
            cm.dispose();
        } else if (player.getClient().getChannel() != 1) { //Only channel 1
        ...
        if (selection == 1) {
			...
			player.getMap().broadcastMessage(Packages.tools.MaplePacketCreator.removeClock());
			player.getMap().broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, expedition.getLeader().getName() + " has ended the expedition."));
