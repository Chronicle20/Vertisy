...
function start() {
    ...
    switch(cm.getPlayer().getMapId()) {
		case 926110200:
			...
			if (cm.haveItem(4001130, 1)) {
				...
				if (em.getProperty("stage4").equals("2")) {
					cm.getPlayer().getMap().getReactorByName("jnr3_out3").hitReactor(cm.getClient());
				}
				cm.getPlayer().getEventInstance().getMapInstance(926110200).getPortal("in00").setPortalState(false);
				cm.getPlayer().getEventInstance().getMapInstance(cm.getPlayer().getMapId()).getReactorByName("jnr3_out1").setState(0);
				...
				cm.getPlayer().getEventInstance().getMapInstance(926110200).getPortal("in01").setPortalState(false);
				cm.getPlayer().getEventInstance().getMapInstance(cm.getPlayer().getMapId()).getReactorByName("jnr3_out2").setState(0);
		...
		case 926110600:
			...
			break;
		case 926100600:
			cm.sendOk("Thank you for helping my love save me. A true hero in a tragedy. Please speak to him, I believe he has something for you.");
			cm.dispose();
			break;
    }
    cm.dispose();
