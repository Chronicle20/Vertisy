...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		if (cm.getJobId() == 200 && cm.getPlayer().getLevel() < 21) {
			...
			cm.sendOk("You are not a magician or you are not under level 21! You may not enter the magician training center!");
			cm.dispose();
		}
    } else if (status == 1) {
