...
function start() {
    ...
    if (isRestingSpot(cm.getPlayer().getMapId())) {
...
function action(mode, type, selection) {
    if (mode === -1) {
        cm.dispose();
    } else if (cm.getPlayer().getMapId() === 925020001) {
        if (mode >= 0) {
            if (status === -1)
            ...
            cm.dispose();
    } else if (isRestingSpot(cm.getPlayer().getMapId())) {
        if (selectedMenu === -1)
            selectedMenu = selection;
        status++;
        if (selectedMenu === 0) {
            cm.warp(cm.getPlayer().getMapId() + 100, 0);
            cm.dispose();
