...
//like the effort put here and becuz of it, i wont remove the hard coded stuffs :D
var stage2Rects = Array(new Packages.java.awt.Rectangle(-755, -132, 4, 218), new Packages.java.awt.Rectangle(-721, -340, 4, 166),
        new Packages.java.awt.Rectangle(-586, -326, 4, 150), new Packages.java.awt.Rectangle(-483, -181, 4, 222));
var stage3Rects = Array(new Packages.java.awt.Rectangle(608, -180, 140, 50), new Packages.java.awt.Rectangle(791, -117, 140, 45),
        new Packages.java.awt.Rectangle(958, -180, 140, 50), new Packages.java.awt.Rectangle(876, -238, 140, 45),
        new Packages.java.awt.Rectangle(702, -238, 140, 45));
var stage4Rects = Array(new Packages.java.awt.Rectangle(910, -236, 35, 5), new Packages.java.awt.Rectangle(877, -184, 35, 5),
        new Packages.java.awt.Rectangle(946, -184, 35, 5), new Packages.java.awt.Rectangle(845, -132, 35, 5),
        new Packages.java.awt.Rectangle(910, -132, 35, 5), new Packages.java.awt.Rectangle(981, -132, 35, 5));
            ...
            if (preamble === null) {
                ...
                if (complete !== null) {
                    ...
                    if (!cm.haveItem(4001008, numpasses)) {
                        ...
                        clear(1, eim, cm);
                        cm.givePartyQuestExp("KerningPQ1st");
...
//        new Rectanglestages(cm); //why new?
        Rectanglestages(cm);
    } else if (curMap === 5) { // Final stage
        var eim = cm.getPlayer().getEventInstance();
        if (eim.getProperty("5stageclear") === null) { //If no
            if (isLeader()) { // Leader
                if (cm.haveItem(4001008, 10) || cm.getPlayer().isGM()) {
                    // Clear stage
                    ...
                    party = eim.getPlayers();
                    cm.getPlayer().isGM() ? cm.gainItem(4001008, 0) : cm.gainItem(4001008, -10);
                    clear(5, eim, cm);
                    cm.givePartyQuestExp("KerningPQFinal");
                } else { // Not done yet
...
function clear(stage, eim, cm) {
    if (stage === 5) { //cleared last stage
        var party = eim.getPlayers();
        for (var i = 0; i < party.size(); i++) {
            party.get(i).endPQ(true, eim.getDuration(), false);
        }
    }
...
//    map = eim.getMapFactory().getMap(103000800 + stage); //?
    ...
    if (isLeader()) { // Check if player is leader
        if (status === 0) {
            ...
            if (preamble === null) { // first time talking.
                cm.sendNext("Hi. Welcome to the " + nthtext
                        + " stage. Next to me, you'll see a number of "
                        + nthobj + ". Out of these " + nthobj
                        ...
                        + nthpos + "; please be near the middle of the "
                        + nthobj + " to be counted as a correct answer. Also, only 3 members of your party are allowed on the "
                        + nthobj + ". Once they are " + nthverb
                ...
                if (cm.getPlayer().getMap().getCharacters().size() !== eim.getPlayerCount()) { //what if other people such as GMs are there?
                ...
                if (complete !== null) {
                    ...
                    if (playersOnCombo === 3 || cm.getPlayer().isGM()) {
                        ...
                        if (correctCombo || cm.getPlayer().isGM()) {
                            clear(curMap, eim, cm);
                            cm.givePartyQuestExp("KerningPQ" + nthtext);
                        } else { // Wrong
                            failstage(eim, cm);
                        }
                    } else {
                        cm.sendNext("It looks like you haven't found the 3 " + nthobj
                                + " just yet. Please think of a different combination of " + nthobj
                                + ". Only 3 are allowed to " + nthverb + " on " + nthobj
                                + ", and if you " + nthpos
...
function isLeader() {
    //no need for these cm.isLeader() already do the job
//    if (cm.getParty() === null)
//        return false;
//    else
    return cm.isLeader();
...
function getPrize(eim, cm) { //this function can be exploited but idc, since it doesnt really matter.
    ...
    else if (itemSetSel < 0.5)
        itemSet = prizeIdEquip;
    else if (itemSetSel < 0.75) {
        itemSet = prizeIdUse;
    ...
    /*if (cm.getPlayer().checkEquippedFor(1072369)) { //shhh :p
        //randomize items if the player is wearing the shoes
//            cm.gainItem(false, itemSet[sel], qty, true, true);

        var PartyQuest = Java.type("server.partyquest.PartyQuest");
        var KPQ = Java.type("server.partyquest.KPQ");
        var pqCount = PartyQuest.getPQCount(cm.getPlayer(), KPQ.class);

        if (pqCount >= 150) {
            item = PartyQuest.gainRandomizedItem(cm.getClient(), itemSet[sel], qty, 1.5);
        } else if (pqCount >= 100) {
            item = PartyQuest.gainRandomizedItem(cm.getClient(), itemSet[sel], qty, 1);
        } else if (pqCount >= 50) {
            item = PartyQuest.gainRandomizedItem(cm.getClient(), itemSet[sel], qty, 0.5);
        } else if (pqCount >= 10) {
            item = PartyQuest.gainRandomizedItem(cm.getClient(), itemSet[sel], qty, 0.1, true); // 0.9x - 1.1x qty
        }
    } else {*/
        item = cm.gainItem(itemSet[sel], qty);
    //}
