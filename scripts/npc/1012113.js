...
function action(mode, type, selection) {
    if (mode < 1) {
        ...
        status++;
        if (cm.getPlayer().getMapId() === 910010100) { //Clear map
            if (status === 0) {
        ...
        } else if (cm.getPlayer().getMapId() === 910010200) { //Bonus map
            if (status === 0) {
        ...
        } else if (cm.getPlayer().getMapId() === 910010300) { //Exit map
            if (status === 0) {
                cm.sendSay("You will now be warped out, thank you for helping us!", false, true);
            } else {
                cm.warp(100000200);
                cm.dispose();
                return;
