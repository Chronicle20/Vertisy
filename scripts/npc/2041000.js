/**
 * @Map: Station<Orbis> (from Ludibrium)
 */
var status = 0;
...
function start() {
	if (cm.haveItem(4031045)) {
		...
		if (em.getProperty("entry") == "true") {
			...
			cm.sendNext("We will begin boarding 5 minutes before the takeoff. Please be patient and wait for a few minutes. Be aware that the train will take off on time, and we stop receiving tickets 1 minute before that, so please make sure to be here on time.");
            cm.dispose();
        }
	} else {
		cm.sendOk("Make sure you got a Orbis ticket to travel in this ship. Check your inventory.");
		cm.dispose();
    }
}
