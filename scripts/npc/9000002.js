...
function action(mode, type, selection) {
    ...
    cm.sendNext("You'll be awarded the #b4,000 NX#k as the winning prize.");
    } else if (status == 2) {
		var loc = pi.getPlayer().getSavedLocation("EVENT");
		if(loc != null)pi.warp(loc);
		else pi.warp(100000000);
		
		cm.getPlayer().getCashShop().gainCash(1, 4000);
		cm.getPlayer().dropMessage(5, "You have gained 4,000 NX!");
