...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getMapId() == 950000000) {
		cm.sendOk("Hi, looks like you're stuck in here.");
		cm.dispose();
		return;
	}
    if (cm.getPlayer().getMapId() == 800000000) {
        if (status == 0) 
            cm.sendSimple("How's the traveling? Are you enjoying it?#b\r\n#L0#Yes, I'm done with traveling. Can I go back to #m" + cm.getPlayer().getSavedLocation("WORLDTOUR").getMapId() + "#?\r\n#L1#No, I'd like to continue exploring this place.");
        else if (status == 1) {
            if (selection == 0) {
	        ...
	        var loc = cm.getPlayer().getSavedLocation("WORLDTOUR");
			if(loc != null)cm.warp(loc);
			else cm.warp(104000000);
            cm.dispose();
        }
    } else {
        if (status == 0) 
