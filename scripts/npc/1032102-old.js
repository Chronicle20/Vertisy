		...
		if (status == 0) {
			...
			if (selection == 0) {
				...
				for (var i = 0; i < 3; i++) {
					currentpet = cm.getPlayer().getPet(i);
					if (currentpet != null && pet == null) {
						if (currentpet.isSummoned() && currentpet.getPetItemId() > 5000047 && currentpet.getPetItemId() < 5000054 && currentpet.getLevel() >= 15) {
							pet = currentpet;
							break;
						}
					}
				...
				var inv = cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.CASH);
			...
			if (theitems.length <= 0) {
				...
				item.setExpiration(cm.getCurrentTime() + (45 * 24 * 60 * 60 * 1000));
