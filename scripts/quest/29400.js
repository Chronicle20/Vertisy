...
function end(mode, type, selection) {
	...
	if (mode == 1)
		...
		status--;
    if(status == 0){
    	qm.sendSay("#L0# Show Leaderboards#l\r\n#L1#Check in#l", false, true);
    }else if(status == 1){
    	if(selection == 0){
    		qm.sendSay("Current Ranking \r\n" + qm.getVeteranHunter() + "\r\nDon't forget that the record resets at the beginning of each month.", false, false);
    	}
    }
}