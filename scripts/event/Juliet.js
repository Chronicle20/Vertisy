...
function setup() {
	...
	var eim = em.newInstance("Juliet_" + em.getProperty("channel"), false);
	...
	firstMap0 = eim.getMapInstance(926110000);
	firstMap0.clearAndReset(false);
	firstMap1 = eim.getMapInstance(926110001);
	firstMap1.clearAndReset(true);
	firstMap = eim.getMapInstance(926110100);
	firstMap.clearAndReset(true);
	secondMap = eim.getMapInstance(926110200);
	secondMap.clearAndReset(true);
    secondMap1 = eim.getMapInstance(926110201);
	secondMap1.clearAndReset(false);
	secondMap1.shuffleReactors();
	secondMap2 = eim.getMapInstance(926110202);
	secondMap2.clearAndReset(false);
	secondMap2.shuffleReactors();
    secondMap3 = eim.getMapInstance(926110203);
	secondMap3.clearAndReset(true);
	thirdMap = eim.getMapInstance(926110300);
	thirdMap.clearAndReset(true);
	thirdMap1 = eim.getMapInstance(926110301);
	thirdMap1.clearAndReset(true);
	thirdMap2 = eim.getMapInstance(926110302);
	thirdMap2.clearAndReset(true);
	thirdMap3 = eim.getMapInstance(926110303);
	thirdMap3.clearAndReset(true);
	thirdMap4 = eim.getMapInstance(926110304);
	thirdMap4.clearAndReset(true);
	fourthMap = eim.getMapInstance(926110400);
	fourthMap.clearAndReset(true);
	fourthMap1 = eim.getMapInstance(926110401);
	fourthMap1.clearAndReset(true);
	fifthMap = eim.getMapInstance(926110500); //spawn Yulete based on properties ?????
	fifthMap.clearAndReset(true);
	sixthMap = eim.getMapInstance(926110600); //spawn romeo&juliet OR fallen romeo/juliet based on properties???
	sixthMap.clearAndReset(true);
	exitMap = em.getChannelServer().getMap(926110700);
...
function changedMap(eim, player, mapid) {
	...
	if (mapid == 926110500) {
		...
		if (em.getProperty("stage7").equals("0")) {
			map.spawnNpc(2112011, 232, 150, map); // Dead Yulete
		} else {
			map.spawnNpc(2112012, 232, 150, map); // Yulete
		}
	}
	if (mapid == 926110600) {
		eim.restartEventTimer(10 * 60 * 1000);
